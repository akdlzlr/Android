...
public class MemoInsertActivity extends AppCompatActivity {
    ...
    TitleBitmapButton mPhotoBtn, mVideoBtn, mVoiceBtn, mHandwritingBtn;

    EditText mMemoEdit;
    ImageView mPhoto;

    String mMemoMode, mMemoId, mMemoDate, mMediaPhotoId, mMediaPhotoUri, mMediaVideoId,
            mMediaVideoUri, mMediaVoiceId, mMediaVoiceUri, mMediaHandwritingId,
            mMediaHandwritingUri, tempPhotoUri, tempVideoUri, tempVoiceUri,
            tempHandwritingUri, mDateStr, mMemoStr;

    Bitmap resultPhotoBitmap, resultHandwritingBitmap;

    boolean isPhotoCaptured, isVideoRecorded, isVoiceRecorded, isHandwritingMade,
            isPhotoFileSaved, isVideoFileSaved, isVoiceFileSaved, isHandwritingFileSaved,
            isPhotoCanceled, isVideoCanceled, isVoiceCanceled, isHandwritingCanceled;

    Calendar mCalendar = Calendar.getInstance();
    TitleBitmapButton insertDateButton, insertTimeButton;

    int mSelectdContentArray;
    int mChoicedArrayItem;

    TitleBitmapButton insertSaveBtn, insertCancelBtn, insert_textBtn,
            insert_handwritingBtn, deleteBtn;
    ...
    private void saveInput() {
        ...
        if (photoFilename != null) {
            // 사진 테이블에 추가된 레코드의 ID값을 확인하기 위한 SQL 정의
        ...
        if (isPhotoCaptured) {
            try {
                if (mMemoMode != null && (mMemoMode.equals(BasicInfo.MODE_MODIFY) || mMemoMode.equals(BasicInfo.MODE_VIEW))) {
                    ...
                    // 기존의 사진 정보를 수정하는 경우에는 테이블에서 레코드 삭제
            ...
            case BasicInfo.REQ_PHOTO_CAPTURE_ACTIVITY:
                ...
                if (resultCode == RESULT_OK) {
                    ...
                    if (isPhotoExists) {
                        ...
                        // 사진 촬영을 통해 저장된 파일을 디코딩하여 비트맵 이미지로 생성
